...
public class AdminCategoryServlet extends BaseServlet{
	/*
	 * 共享前台的CategoryService和CategroyDao
	 */
	...
	/**
	 * 添加一级分类
	 * 注意：这里添加的时候会有一个bug，如果添加的类别在数据库中已经存在，就会存在SQL异常：
	 * Duplicate insert:例如： Duplicate entry 'Hadoop' for key 'cname'
	 * @param req
	 * @param resp
	 * @return
	 * @throws ServletException
	...
	public String addParent(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 3.调用findAll(),返回list.jsp显示所有的分类
		 */
		
		String cname = req.getParameter("cname");
		int number = categoryService.findCountByCname(cname);
		if(number > 0) {
			req.setAttribute("msg", "该分类已经存在，请不要重复添加！");
	...
	public String addChildPre(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		String pid = req.getParameter("pid");//也是当前点击的父分类cid
		List<Category> parents = categoryService.findParents();
		req.setAttribute("pid", pid);
		req.setAttribute("parents", parents);
		
		return "f:/adminjsps/admin/category/add2.jsp";//转发到add2.jsp
	}
	...
	public String addChild(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 3.调用service层的add()方法完成添加
		 * 4.调用findAll()，返回list.jsp显示所有的分类
		 */
	...
	public String deleteParent(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException,IOException {
		/*
		 * 1.获取链接参数cid,它是一个一级分类的cid
		 * 2.通过cid，查看该父分类下子分类的个数
		 * 3.如果大于零，说明还有子分类，不能删除；保存错误信息，转发到msg.jsp
		 * 4.如果等于零，则可以删除它，返回list.jsp
		 */
		String cid = req.getParameter("cid");
		int number = categoryService.findChildCountByParent(cid);
		if(number > 0){
