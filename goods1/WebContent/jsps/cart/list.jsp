...
$(function() {  //页面加载完成之后，自动加载该函数
	...
	*/
	 ...
	 */
	 $(":checkbox[name=checkboxBtn]").click(function() {
		var all = $(":checkbox[name=checkboxBtn]").length;//所有条目复选框的的个数
		var select = $(":checkbox[name=checkboxBtn][checked=true]").length;//被选中的条目的个数
		if(all == select) {//全选中了
			$("#selectAll").attr("checked", true);//勾选全选复选框
			setJieSuan(true);//让结算按钮有效
		} else if(select == 0) {//全部不选
			$("#selectAll").attr("checked", false);//取消全选
			setJieSuan(false);//结算按钮失效
		} else {
			$("#selectAll").attr("checked", false);//取消全选
			setJieSuan(true);//结算按钮依然有效
		}
		showTotal();//重新计算总计
	 });
	...
	*/
	$(".jian").click(function() {
		//获取cartItemId
		var id = $(this).attr("id").substring(0,32);//从id中提取出来cartItemId
		//获取输入框中的数量
		var quantity = $("#" + id + "Quantity").val();
		//判断当前数量是否为1，如果为1，那就不是修改数量了，而是要删除该条目
		if(quantity == 1) {
			if(confirm("您是否要删除该条目？")) {
				location = "/goods1/CartItemServlet?method=batchDelete&cartItemIds=" + id;
			}
		} else {
			sendUpdateQuantity(id, Number(quantity) - 1);//构造修改方法
		}
	});
...
//请求服务器，修改数量(url：请求->if success->result接收json对象)
...
function showTotal() {
	...
	*/
	$(":checkbox[name=checkboxBtn][checked=true]").each(function() {
		//2.获取复选框的值，即cartItemId,也是其他元素的前缀
		var id = $(this).val();
		//3.用前缀构造小计的id，并获取它的文本值（String）
 ...
 */
function jiesuan() {
	//1.将所有被选中的cartItem的id，放到数组中
	var cartItemIdArray = new Array();
	$(":checkbox[name=checkboxBtn][checked=true]").each(function() {
		cartItemIdArray.push($(this).val());//将复选框的值，即id添加到数组中
	});
	//2.把数组的值转换成字符串，然后赋值给表单的<id=cartItemIds>
	...
	<c:forEach items="${cartItemList }" var="cartItem">
		...
		<tr>
			...
			<td colspan="3" align="right" class="tdTotal">
				<span>总计：</span><span class="price_t">&yen;<span id="total"></span></span>   <%--总计:设置total的文本内容：showTotal() --%>
			</td>
		</tr>
		<tr>
