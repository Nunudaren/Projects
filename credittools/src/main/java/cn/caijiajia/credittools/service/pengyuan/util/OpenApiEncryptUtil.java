...
public class OpenApiEncryptUtil {
    ...
    public static boolean verifySignByPublicKey4MicroSoft(String signData, String partnerPublicKey, String sign) throws Exception {
        return verifySignByPublicKey(signData, partnerPublicKey, sign);
    }

    public static String decryptByRSA4MicroSoft(String inputStr, String privateKey) throws Exception {
        PrivateKey key = getPrivateKeyFromString(privateKey);
        return new String(decryptByRSA4MicroSoft(Base64.decodeBase64(inputStr), key));
    }

    private static byte[] decryptByRSA4MicroSoft(byte[] input, PrivateKey key) throws Exception {
    ...
    public static String encryptByRSA4MicroSoft(String inputStr, String publicKey) throws Exception {
        PublicKey key = getPublicKeyFromString(publicKey);
        return Base64.encodeBase64String(encryptByRSA4MicroSoft(inputStr.getBytes(), key));
    }

    private static byte[] encryptByRSA4MicroSoft(byte[] input, PublicKey key) throws Exception {
        Security.addProvider(new BouncyCastleProvider());
        Cipher cipher = Cipher.getInstance("RSA/NONE/NoPadding");
        cipher.init(1, key);
        byte[] output = cipher.doFinal(input);
